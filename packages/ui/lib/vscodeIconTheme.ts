...
export class VSCodeIconThemeResolver {
  ...
  resolveIcon(
    ...
    if (isDirectory) {
      ...
      if (fileNames?.[name]) {
        ...
        const fileExtensions = this.getEffectiveMap(
          json.fileExtensions,
  ...
  async loadIcon(iconPath: string): Promise<string | null> {
    ...
    const promise = (async () => {
      try {
        const buf = await readFileBuffer(this.bridge, iconPath);
...
export const vscodeIconThemeAtom = atom((get) => {
  const bridge = get(bridgeAtom);
  if (!bridge) {
    throw new Error("Bridge not initialized.");
  }
  return new VSCodeIconThemeResolver(bridge);
});

export function useVscodeIconTheme(): VSCodeIconThemeResolver {
  return useAtomValue(vscodeIconThemeAtom);
}