/**
 * Minimal Monaco entry point — editor core + contrib, no languages.
 *
 * This is a forked copy of `monaco-editor/esm/vs/editor/editor.main.js` with
 * two categories of imports intentionally removed:
 *
 *   1. `vs/basic-languages/*` — ~90 Monarch tokenizers (abap, apex, bicep, …)
 *      that each add a dedicated chunk to the bundle. Syntax highlighting in
 *      this app is driven by TextMate grammars contributed by extensions and
 *      tokenized via `vscode-textmate`, so these Monarch ones are dead weight.
 *
 *   2. `vs/language/{css,html,json,typescript}/monaco.contribution` — the
 *      built-in JSON/CSS/HTML/TypeScript language services. Each one bundles
 *      its own language server into a dedicated Web Worker (adding megabytes
 *      to the final asset). In the VS Code-compatible model we target,
 *      language services come from installed extensions, not the editor core.
 *
 * Everything else — `editor.api`, cursor/selection commands, find, folding,
 * hover, suggest, rename, snippets, gotoSymbol, inlayHints, semantic tokens,
 * quick access, etc. — is still imported so the standalone editor feels
 * complete.
 */

// Re-export the public API from `editor.api.js` (which itself just re-exports
// from the un-typed `editor.api2.js` but ships with .d.ts declarations). This
// matches the surface `monaco-editor/esm/vs/editor/editor.main.js` exposes
// without pulling in any language contributions.
